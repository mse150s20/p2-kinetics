...
def kinetics(rateInfect, rateRecover, h_0, i_0, r_0):
    ...
    def abc(t, y):
        ...
        return [
            -rateInfect * healthy * infected / (healthy + infected + recovered),
            rateInfect * healthy * infected / (healthy + infected + recovered)
            - rateRecover * infected,
            rateRecover * infected,
        ]
...
plt.plot(solution.t, solution.y[0], label="Model Susceptible")
plt.plot(solution.t, solution.y[1], label="Model Infected")
plt.plot(solution.t, solution.y[2], label="Model Recovered")
plt.legend()
plt.xlabel("Time [days]")
